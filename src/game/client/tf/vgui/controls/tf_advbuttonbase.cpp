...
void CTFButtonBase::RunAnimation(void)
{
	return;
}
...
bool CTFButtonBase::RequestInfo( KeyValues *data )
{
	...
	if ( !V_stricmp( data->GetName(), "textinsetx" ) )
	{
		data->SetInt( "textinsetx", (int)data );
		return true;
	}
	else if ( !V_stricmp( data->GetName(), "textinsety" ) )
	{
		data->SetInt( "textinsety", (int)data );
		return true;
	}
...
{
	if (!V_stricmp( inputData->GetName(), "textinsetx" ))
	{
		inputData->GetInt( "textinsetx", 0 );
		return true;
	}
	else if (!V_stricmp( inputData->GetName(), "textinsety" ))
	{
		inputData->GetInt( "textinsety", 0 );
		return true;
	}
