...
CTFSpectatorGUI::CTFSpectatorGUI(IViewPort *pViewPort) : CSpectatorGUI(pViewPort)
{
	...
	m_nDemoBuilds_xpos = m_nEngBuilds_xpos = m_nEngBuilds_ypos = 0;
	m_nDemoBuilds_ypos = m_nSpyBuilds_xpos = m_nSpyBuilds_ypos = 0;
...
void CTFSpectatorGUI::ShowPanel(bool bShow)
{
	if ( bShow != IsVisible() )
	{
		...
		CHudBuildingStatusContainer_Demoman* pDemoBuilds = GET_NAMED_HUDELEMENT( CHudBuildingStatusContainer_Demoman, BuildingStatus_Demoman );
		...
		if ( bShow )
		{
			...
			if ( pDemoBuilds )
			{
				pDemoBuilds->GetPos( xPos, yPos );
				m_nDemoBuilds_xpos = xPos;
				m_nDemoBuilds_ypos = yPos;
				pDemoBuilds->SetPos( xPos, GetTopBarHeight() );
			}
		...
		else
		{
			...
			if ( pDemoBuilds )
			{
				pDemoBuilds->SetPos( m_nDemoBuilds_xpos, m_nDemoBuilds_ypos );
			}
		}
