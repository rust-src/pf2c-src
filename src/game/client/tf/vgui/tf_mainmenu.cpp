...
#include "panels/pf_creditspanel.h"
...
CON_COMMAND(pf2_mainmenu_reload, "Reload Main Menu")
{
...
CTFMainMenu::CTFMainMenu(VPANEL parent) : vgui::EditablePanel(NULL, "MainMenu")
{
	...
	AddMenuPanel( new CTFCreditsPanel( this, "CTFCreditsPanel" ), CREDITSDIALOG_MENU );
...
void CTFMainMenu::SetDelayedShow(MenuPanel panel, float time)
{
	bDelayedShow = true;
	flDelayedShowTime = time + gpGlobals->curtime;
	iDelayedShowPanel = panel;

}

void CTFMainMenu::SetDelayedHide(MenuPanel panel, float time)
{
	bDelayedHide = true;
	flDelayedHideTime = time + gpGlobals->curtime;
	iDelayedHidePanel = panel;
}
...
void CTFMainMenu::OnTick()
{
	...
	if (bDelayedHide)
	{
		if (flDelayedHideTime < gpGlobals->curtime)
		{
			HidePanel(iDelayedHidePanel);
			bDelayedHide = false;
		}
	}
	if (bDelayedShow)
	{
		if (flDelayedShowTime < gpGlobals->curtime)
		{
			ShowPanel(iDelayedShowPanel);
			bDelayedShow = false;
		}
	}
