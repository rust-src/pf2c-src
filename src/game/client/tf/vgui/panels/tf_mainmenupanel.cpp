...
#define BLOG_URL "https://verysuspicioussuspect.github.io/updates_page01.html"

ConVar pf2_mainmenu_music("pf2_mainmenu_music", "1", FCVAR_ARCHIVE, "Toggle music in the main menu");
...
void CTFMainMenuPanel::PerformLayout()
{
	...
	if (m_pProfileAvatar)
	{
		m_pProfileAvatar->SetPlayer(m_SteamID, k_EAvatarSize184x184);
...
void CTFMainMenuPanel::OnCommand(const char* command)
{
	...
	{
		MAINMENU_ROOT->ShowPanel(CREDITSDIALOG_MENU);
	}
...
void CTFMainMenuPanel::OnTick()
{
	...
	if (pf2_mainmenu_music.GetBool() && !bInGameLayout)
	{
