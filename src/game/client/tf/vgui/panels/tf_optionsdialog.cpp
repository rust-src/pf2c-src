...
bool CTFOptionsDialog::Init()
{
	...
	AddPanel( new CTFOptionsMultiplayerPanel( this, "MultiplayerOptions" ), PANEL_MULTIPLAYER );
	...
	AddPanel( new CTFPFOptionsPanel( this, "PF2Options" ), PANEL_PF2 );
	//AddPanel( new CTFOptionsAdvancedPanel( this, "AdvancedOptions" ), PANEL_ADV );
	m_pApplyButton = new CTFButton( this, "Apply", "" );
	m_pDefaultsButton = new CTFButton( this, "Defaults", "" );

	m_pOptionsCurrent = PANEL_MOUSE;
...
void CTFOptionsDialog::OnCommand(const char* command)
{
	...
	else if (!Q_strcmp(command, "newoptionsmultiplayer"))
	{
		SetCurrentPanel(PANEL_MULTIPLAYER);
	}
	...
	else if (!Q_strcmp( command, "newoptionspf2" ))
	{
		SetCurrentPanel( PANEL_PF2 );
	}
