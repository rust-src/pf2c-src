...
private:
	...
	CPanelAnimationVar( Color, m_clrLocalText, "LocalPlayerColor", "65 65 65 255" );
...
void CTFHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		if ( msg.wzInfoText[0] )
		{
			...
			DrawText( x + iDeathInfoOffset, yText, m_hTextFont, msg.bLocalPlayerInvolved ? m_clrLocalText : Color( 255, 255, 255, 255 ), msg.wzInfoText );
...
void CTFHudDeathNotice::OnGameEvent( IGameEvent *event, int iDeathNoticeMsg )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		case TF_DMG_CUSTOM_BURNING:
			...
			msg.wzInfoText[0] = 0;
			break;
		case TF_DMG_CUSTOM_NAPALM_BURNING:
			Q_strncpy( msg.szIcon, "d_napalm_fire", ARRAYSIZE( msg.szIcon ) );
			msg.wzInfoText[ 0 ] = 0;
			break;
