...
CON_COMMAND_F( pf2_checkmessages, "Check for the messages", FCVAR_DEVELOPMENTONLY )
{
	GetNotificationManager()->CheckVersionAndMessages();
}

CON_COMMAND_F( pf2_updateserverlist, "Check for the messages", FCVAR_DEVELOPMENTONLY )
{
	GetNotificationManager()->UpdateServerlistInfo();
}

ConVar pf2_checkfrequency( "pf2_checkfrequency", "900", FCVAR_DEVELOPMENTONLY, "Messages check frequency (seconds)" );
ConVar pf2_updatefrequency( "pf2_updatefrequency", "15", FCVAR_DEVELOPMENTONLY, "Updatelist update frequency (seconds)" );
ConVar pf2_latest_notification( "pf2_latest_notification", "0", FCVAR_ARCHIVE );
...
bool CTFNotificationManager::Init()
{
	if ( !m_bInited )
	{
		...
		m_flLastCheck = pf2_checkfrequency.GetFloat() * -1;
		m_flUpdateLastCheck = pf2_updatefrequency.GetFloat() * -1;
...
void CTFNotificationManager::Update( float frametime )
{
	...
	if ( gpGlobals->curtime - m_flLastCheck > pf2_checkfrequency.GetFloat() )
	{
		m_flLastCheck = gpGlobals->curtime;
		CheckVersionAndMessages();
	}

	if ( !MAINMENU_ROOT->InGame() && gpGlobals->curtime - m_flUpdateLastCheck > pf2_updatefrequency.GetFloat() )
	{
...
void CTFNotificationManager::OnMessageCheckCompleted( const char *pszPage )
{
	...
	time_t timePrevious = V_atoi64( pf2_latest_notification.GetString() );
	time_t timeNew = V_atoi64( TF_NOTIFICATION_TEST_TIME );

	if ( timeNew <= timePrevious ) // Already viewed this one.
		return;

	pf2_latest_notification.SetValue( TF_NOTIFICATION_TEST_TIME );
