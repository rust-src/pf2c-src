...
BEGIN_DMXELEMENT_UNPACK( RoundStats_t )
	...
	DMXELEMENT_UNPACK_FIELD( "iArmorPointsRepaired", "0", int, m_iStat[ TFSTAT_ARMORRESTORED ] )
	DMXELEMENT_UNPACK_FIELD( "iHealthAltPointsHealed", "0", int, m_iStat[ TFSTAT_HEALING_OTHER ] )
...
TFStatType_t g_statPriority[] = { TFSTAT_HEADSHOTS, TFSTAT_BACKSTABS, TFSTAT_MAXSENTRYKILLS, TFSTAT_HEALING, TFSTAT_ARMORRESTORED, TFSTAT_KILLS, TFSTAT_KILLASSISTS,
...
void CTFStatPanel::UpdateStats( int iMsgType )
{
	...
	if ( m_statRecord > TFSTAT_UNDEFINED )
	{
		...
		if ( !bAlive || ( gpGlobals->curtime - m_flTimeLastSpawn < 3.0 ) )
		{
			...
			m_flTimeHide = gpGlobals->curtime + ( bAlive ? 8.0f : 12.0f );
			m_statRecord = TFSTAT_UNDEFINED;
		}
	}
