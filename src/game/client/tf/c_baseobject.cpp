...
IMPLEMENT_CLIENTCLASS_DT(C_BaseObject, DT_BaseObject, CBaseObject)
	...
	RecvPropInt(RECVINFO(m_bEnemyUsing)),
...
bool C_BaseObject::IsEnemyUsing(void)
{
	return m_bEnemyUsing;
}
...
void C_BaseObject::GetTargetIDString( wchar_t *sIDString, int iMaxLenInBytes )
{
	...
	if ( iTeamNumber == pLocalPlayer->GetTeamNumber() || pLocalPlayer->IsPlayerClass(TF_CLASS_SPY))
	{
...
{
	// This shouldn't ever happen, but it might
	if (GetMaxHealth() == 0)
		return BUILDING_HUD_ALERT_NONE;
	...
	if (IsEnemyUsing())
	{
		alertLevel = BUILDING_HUD_ALERT_ENEMY_USING;
	}
	else if ( HasSapper() )
	{
