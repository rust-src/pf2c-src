...
CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectTFBloodSpray )
...
//			scale -
//-----------------------------------------------------------------------------
void FX_TF_BloodSpray( const Vector &origin, const Vector &normal, float flDamage )
{
	if (pf_blood_impact_disable.GetBool())
		return;
...
{
	FX_TF_BloodSpray( data.m_vOrigin, data.m_vNormal, data.m_flMagnitude );
}

DECLARE_CLIENT_EFFECT( "TFblood", TFBloodSprayCallback );
