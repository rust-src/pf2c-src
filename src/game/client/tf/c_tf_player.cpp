...
void CSpyInvisProxy::OnBind( C_BaseEntity *pEnt )
{
	...
	if( pf_team_colored_spy_cloak.GetBool() )
	{
...
void C_TFPlayer::ClientThink()
{
	...
	if( m_pSleepyEffect )
	{
		...
		{
			ParticleProp()->StopEmissionAndDestroyImmediately( m_pSleepyEffect );
			m_pSleepyEffect = NULL;
		}
	}
...
void C_TFPlayer::CreateSaveMeEffect( void /*bool bAutoCall*/)
{
	...
	if( pLocalPlayer && pLocalPlayer->IsPlayerClass( TF_CLASS_MEDIC ) && pLocalPlayer->IsAlive() == true )
	{
		...
		if( GetAttachmentLocal( LookupAttachment( "head" ), vecPos ) )
		{
			...
			MedicCallType_t callType = MEDICCALL_DEFAULT;
...
void C_TFPlayer::UpdateTypingBubble( void )
{
	...
	if (InFirstPersonView() || m_Shared.InCond(TF_COND_TAUNTING))
		return;

	bool bTeamSay = !( GetTFChatHud()->GetMessageMode() == MM_SAY );

	// for all chat, show it over anyone's head.
	if (m_bTyping && IsAlive() && ( bTeamSay ? !IsEnemyPlayer() : !(m_Shared.IsStealthed() && IsEnemyPlayer() ) ) )
	{
		if( !m_pTypingEffect )
...
{
	SetContextThink(&C_TFPlayer::DoHallucination, gpGlobals->curtime + RandomFloat(0.0f, 3.0f), "HallucinationThink");
	...
	case TF_HALLUCINATION_GRENADE:
		EyeVectors(&forward);
		UTIL_TraceLine(Weapon_ShootPosition(), Weapon_ShootPosition() + (forward * 300), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
		...
		pGrenade = tempents->SpawnTempModel(model, Vector(tr.endpos.x, tr.endpos.y, tr.endpos.z + 70.0f), GetAbsAngles(), Vector(-GetAbsVelocity().x, -GetAbsVelocity().y, GetAbsVelocity().z), 5.0f, FTENT_COLLIDEALL);
		if (!pGrenade)
			break;
