...
void CTFHudObjectiveStatus::SetVisiblePanels( void )
{
	...
	else if ( TFGameRules()->GetGameType() == TF_GAMETYPE_CP )
	{
		...
		// Yeah this isn't mess. Don't worry about it.
		if (m_pFlagPanel)
		{
			if (m_pFlagPanel->IsVisible() && (TFGameRules()->GetADFlagMode() == AD_FLAGMODE_AVAILABLE))
			{
				m_pFlagPanel->SetVisible( false );
			}
			else if (TFGameRules()->GetADFlagMode() != AD_FLAGMODE_AVAILABLE )
			{
				if (m_pFlagPanel->IsVisible() && !TFGameRules()->IsCTFCPHybrid())
				{
					m_pFlagPanel->SetVisible( false );
				}
				else if (!m_pFlagPanel->IsVisible() && TFGameRules()->IsCTFCPHybrid())
				{
					m_pFlagPanel->SetVisible( true );
				}
			}
