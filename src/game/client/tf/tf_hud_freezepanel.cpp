...
DECLARE_BUILD_FACTORY( CTFFreezePanelArmor );
...
void CTFFreezePanel::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	{
		// Normal
		m_pFreezeLabel[ 0 ] = dynamic_cast<Label *>( m_pBasePanel->FindChildByName("FreezeLabel") );
		m_pFreezeLabelKiller[ 0 ] = dynamic_cast<Label *>( m_pBasePanel->FindChildByName("FreezeLabelKiller") );
		m_pAvatar[ 0 ] = dynamic_cast<CAvatarImagePanel *>( m_pBasePanel->FindChildByName("AvatarImage") );

		// Armor
		m_pFreezeLabel[ 1 ] = dynamic_cast< Label* >( m_pBasePanel->FindChildByName( "FreezeLabelArmor" ) );
		m_pFreezeLabelKiller[ 1 ] = dynamic_cast< Label* >( m_pBasePanel->FindChildByName( "FreezeLabelKillerArmor" ) );
		m_pAvatar[ 1 ] = dynamic_cast< CAvatarImagePanel* >( m_pBasePanel->FindChildByName( "AvatarImageArmor" ) );
		...
		m_pKillerArmor = dynamic_cast<CTFFreezePanelArmor *>( m_pBasePanel->FindChildByName( "FreezePanelArmor" ) );
	...
	m_pFreezeCamModel = dynamic_cast<CModelPanel *>( FindChildByName( "FreezePanelModel" ));
...
// Purpose: 
//-----------------------------------------------------------------------------
//void CTFFreezePanel::ApplySettings( KeyValues *inResourceData )
//{
...
void CTFFreezePanel::FireGameEvent( IGameEvent * event )
{
	...
	else if ( Q_strcmp( "show_freezepanel", pEventName ) == 0 )
	{
		...
		// Reset freezecam model frame visiblity
		m_pFreezeCamModel->SetVisible( false );
		m_bFreezeTimeExtended = false;
		...
		if ( pKiller )
		{
			...
			// With the unfortunate truth that is people turning off armor, this has to exist
			vgui::Label* pFreezeLabel;
			CAvatarImagePanel* pAvatar;
			bool bArmor = !pKiller->IsBaseObject() && TFGameRules()->GetArmorMode();
			if ( bArmor )
			{
				int iMaxArmor = 1;
				int iArmor = 0;
				int iArmorType = ARMOR_NONE;
				if ( pPlayer )
				{
					iArmor = pPlayer->ArmorValue();
					iMaxArmor = pPlayer->GetPlayerClass()->GetMaxArmor();
					iArmorType = pPlayer->GetPlayerClass()->GetArmorType();
				}
				if ( !pKiller->IsAlive() )
				{
					iArmor = 0;
				}
				m_pKillerArmor->SetArmor( iArmor, iMaxArmor );
				m_pKillerArmor->SetArmorImageTeam( pKiller->GetTeamNumber(), iArmorType );
				m_pKillerArmor->SetVisible( true );
			}
			else
			{
				m_pKillerArmor->SetVisible( false );
			}

			// Set control visibility and pointers
			m_pFreezeLabel[ !bArmor ]->SetVisible( false );
			m_pFreezeLabelKiller[ !bArmor ]->SetVisible( false );
			m_pAvatar[ !bArmor ]->SetVisible( false );
			
			m_pFreezeLabel[ bArmor ]->SetVisible( true );
			m_pFreezeLabelKiller[ bArmor ]->SetVisible( true );
			m_pAvatar[ bArmor ]->SetVisible( true );

			pFreezeLabel = m_pFreezeLabel[ bArmor ];
			pAvatar = m_pAvatar[ bArmor ];
...
void CTFFreezePanel::OnThink( void )
{
	...
	if (m_bHoldingAfterScreenshot && !engine->IsTakingScreenshot())
	{
		if ( pf_freezecammodel.GetBool() && m_pFreezeCamModel && !m_pFreezeCamModel->IsVisible())
		{
			
			extern bool IsTakingAFreezecamScreenshot();

			m_pFreezeCamModel->SetVisible( true );
			KeyValues *kvParms = new KeyValues( "SetAnimation" );
			if (kvParms)
			{
				char pszAnimation[ 32 ];
				Q_snprintf( pszAnimation, sizeof( pszAnimation ), "drop0%d", RandomInt( 1, 4 ) );
				kvParms->SetString( "animation", pszAnimation );
				PostMessage( m_pFreezeCamModel, kvParms );
			}
		}
	}
}
	...
	if ( ShouldDraw() && pszCurrentBinding )
	{
		if ( FStrEq( pszCurrentBinding, "screenshot" ) || FStrEq( pszCurrentBinding, "jpeg" ) )
		{
			...
			if (pPlayer)
			{
				...
				if (!m_bFreezeTimeExtended)
				{
					engine->ClientCmd( "extendfreeze" );
					view->FreezeFrame( 3.0f );
					m_bFreezeTimeExtended = true;
				}
