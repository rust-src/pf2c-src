...
void CTFMedicCallerPanel::PerformLayout( void )
{
	...
	// "CallerAuto", "CallerBurning", "CallerBleeding" and "CallerHealth"(means low HP)

	vgui::Panel *pPanel = FindChildByName( "CallerBG" );
	if ( pPanel )
	{
		pPanel->SetVisible( m_iType != MEDICCALL_LOWHEALTH || m_iType != MEDICCALL_BURNING );
		pPanel->SetPos( (GetWide() - pPanel->GetWide()) * 0.5, (GetTall() - pPanel->GetTall()) * 0.5 );
	}

	pPanel = FindChildByName( "CallerHealth" );
	if ( pPanel )
	{
		pPanel->SetVisible( m_iType == MEDICCALL_LOWHEALTH || m_iType == MEDICCALL_BURNING );
		pPanel->SetPos( ( GetWide() - pPanel->GetWide() ) * 0.5, ( GetTall() - pPanel->GetTall() ) * 0.5 );
	...
	pPanel = FindChildByName( "CallerBurning" );
	if ( pPanel )
	{
		bool bVisible = m_iType == MEDICCALL_BURNING;
		pPanel->SetVisible( bVisible );
		if ( bVisible )
			SetSize( pPanel->GetWide(), pPanel->GetTall() );
		pPanel->SetPos( ( GetWide() - pPanel->GetWide() ) * 0.5, ( GetTall() - pPanel->GetTall() ) * 0.5 );
		
	}

	pPanel = FindChildByName( "CallerBleeding" );
	if ( pPanel )
	{
		bool bVisible = m_iType == MEDICCALL_BLEEDING;
		pPanel->SetVisible( bVisible );
		if ( bVisible )
			SetSize( pPanel->GetWide(), pPanel->GetTall() );
		pPanel->SetPos( ( GetWide() - pPanel->GetWide() ) * 0.5, ( GetTall() - pPanel->GetTall() ) * 0.5 );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFMedicCallerPanel::AddMedicCaller( C_TFPlayer *pPlayer, float flDuration, Vector &vecOffset, MedicCallType_t iType /*= 0*/ )
{
...
{
	if ( FStrEq( "player_calledformedic", event->GetName() ) )
	{
		// Do something IDK
		// Probably have to call AddMedicCaller with the event info
	}
}
