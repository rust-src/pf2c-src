...
void CTFHudGrenadeAmmo::OnThink()
{
	...
	if (m_flNextThink < gpGlobals->curtime)
	{
		...
		if (!tfPlayer || !pf_grenades.GetBool())
		{
			UpdateGrenadesLabels( false, false );
			m_nGrenades1 = 0;
			m_nGrenades2 = 0;
		...
		{
			C_TFPlayerClass *c = tfPlayer->GetPlayerClass();
			if (c)
			{
				TFPlayerClassData_t *d = c->GetData();
				if (d)
				{
					UpdateGrenadesLabels( d->m_aAmmoMax[TF_AMMO_GRENADES1] > 0, d->m_aAmmoMax[TF_AMMO_GRENADES2] > 0 );
					m_nGrenades1 = tfPlayer->GetAmmoCount( TF_AMMO_GRENADES1 );
					m_nGrenades2 = tfPlayer->GetAmmoCount( TF_AMMO_GRENADES2 );
				}
			}
		}
