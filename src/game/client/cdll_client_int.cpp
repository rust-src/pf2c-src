...
#ifdef PF2_CLIENT
void pf_picmip_helper( IConVar* var, const char* pOldValue, float flOldValue )
{
...
void mat_picmip_helper( IConVar* var, const char* pOldValue, float flOldValue )
{
	...
	{
		int iPicmip = mat_picmip->GetInt();
		if( iPicmip != pf_picmip->GetInt() )
			pf_picmip->SetValue( iPicmip );
	}
}
static ConVar cl_load_tf2_content( "cl_load_tf2_content", "1", FCVAR_ARCHIVE, "Mount the content from Team Fortress 2 if possible" );
#endif
...
void CHLClient::PostInit()
{
	...
	if ( mat_picmip )
	{
		...
		mat_picmip->SetMax( 10.0f );
#ifdef OSX
		// Change mat_picmip limits
		if ( pf_picmip.GetInt() != 0 )
		{
			// Don't want to make an OSX engine patch so just work like before
			mat_picmip->SetValue( pf_picmip.GetInt() );
			// delete the convar
			mat_picmip->Nuke();
	}
#else
		mat_picmip->AddFlags( FCVAR_HIDDEN );
		mat_picmip->InstallChangeCallback( mat_picmip_helper );
#endif
	}
#endif
}
