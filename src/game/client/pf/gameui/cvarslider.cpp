...
void CCvarSlider::SetupSlider( float minValue, float maxValue, const char *cvarname, bool bAllowOutOfRange )
{
	...
	if ( ( m_flMaxValue - m_flMinValue ) > 1.0f )
		m_fScaleFactor = 1.0f;
	else
		m_fScaleFactor = CVARSLIDER_SCALE_FACTOR;

	// scale by CVARSLIDER_SCALE_FACTOR
	SetRange( (int)( m_fScaleFactor * minValue ), (int)( m_fScaleFactor * maxValue ) );
...
//	float curvalue = engine->pfnGetCvarFloat( m_szCvarName );
    ...
    {
        int val = (int)( m_fScaleFactor * curvalue );
...
void CCvarSlider::ApplyChanges()
{
    if (m_bModifiedOnce)
    {
        ...
        {
            m_fStartValue = (float) m_iStartValue / m_fScaleFactor;
        }
...
{
    int nVal = (int)( m_fScaleFactor * fValue );
    SetValue( nVal, false);
...
//	m_fStartValue = engine->pfnGetCvarFloat( m_szCvarName );
    ...
    int value = (int)( m_fScaleFactor * m_fStartValue );
	SetValue( value );
...
void CCvarSlider::OnSliderMoved()
{
	if (HasBeenModified())
	{
        ...
        {
            m_iLastSliderValue = GetValue();
            m_fCurrentValue = ((float) m_iLastSliderValue)/ m_fScaleFactor;
        }
