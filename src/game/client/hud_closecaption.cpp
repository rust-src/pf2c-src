...
void OnCaptionLanguageChanged( IConVar *pConVar, const char *pOldString, float flOldValue )
{
	...
	Q_snprintf(fn2, sizeof(fn2), "resource/pf2beta_%s.txt", var.GetString());
	...
	{
		g_pVGuiLocalize->AddFile( "resource/closecaption_%language%.txt", "GAME", true );
		g_pVGuiLocalize->AddFile("resource/pf2beta_%language%.txt", "GAME", true);
	...
	if ( Q_strlen( var.GetString() ) > 0 && Q_stricmp( var.GetString(), uilanguage ) )
	{
		if ( !IsX360() )
		{
			if ( g_pFullFileSystem->FileExists( fn ) )
			{
				...
				g_pVGuiLocalize->AddFile( fn, "GAME" );
			...
			{
				char dbfile[512];
				Q_snprintf(dbfile, sizeof(dbfile), "resource/pf2beta_%s.txt", var.GetString());
				g_pVGuiLocalize->AddFile(fn2, "GAME", true);
			}
			else
			{
				char fallback[512];
				Q_snprintf(fallback, sizeof(fallback), "resource/pf2beta_%s.txt", uilanguage);
	...
	DevMsg( "custom_lang = %s\n", var.GetString() );
}



ConVar lang_file( "lang_file", "", FCVAR_ARCHIVE, "Current language file to use (emtpy = use game UI language)", OnCaptionLanguageChanged );
