...
BEGIN_DATADESC( CTeamControlPointMaster )
	...
	DEFINE_KEYFIELD( m_iszAssociatedFlag, FIELD_STRING, "cp_ad_flag" ),
...
//	DEFINE_FIELD( m_bFirstRoundAfterRestart, FIELD_BOOLEAN ),
	...
	DEFINE_OUTPUT(m_outputOnCPFlagEnabled, "OnCPFlagEnabled"),
	DEFINE_OUTPUT(m_outputOnCPFlagDisabled, "OnCPFlagDisabled"),
...
void CTeamControlPointMaster::Spawn( void )
{
	...
	SetupADFlagMode( !pf_cp_flag.GetBool() );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::SetupADFlagMode(bool bOldValue)
{
	if (bOldValue != pf_cp_flag.GetBool())
	{
		if (pf_cp_flag.GetBool())
		{
			m_outputOnCPFlagEnabled.FireOutput(this, this);
		}
		else
		{
			m_outputOnCPFlagDisabled.FireOutput(this, this); 
		}
	}
	
}

void CTeamControlPointMaster::UpdateADFlagMode( bool bOldValue )
{
	if (bOldValue != pf_cp_flag.GetBool())
	{
		if(m_iCurrentRoundIndex != -1)
			m_ControlPointRounds[m_iCurrentRoundIndex]->HandleADFlag();
		SetupADFlagMode( bOldValue );
		HandleADFlag();
		TFGameRules()->SetADFlagMode( pf_cp_flag.GetBool() );
	}
}
...
bool CTeamControlPointMaster::FindADFlag( void )
{
...
void CTeamControlPointMaster::HandleADFlag( void )
{
	...
	SetupADFlag( pf_cp_flag.GetBool() );
}

void CTeamControlPointMaster::SetupADFlag( bool bEnabled )
{
	if (!m_hAssociatedFlag)
		return;

	// Can't kill the flag, so just force the player holding it to drop it.
	// this isn't perfect as the flag countdown effect remains where the flag was last if the convar is re-enabled, until it is picked up again.
	// This is still preferable than permenantly impacting a player's experience.
	if (!m_hAssociatedFlag->IsHome())
	{
		bool bFlagDropped = false;
		for (int i = 0; i < gpGlobals->maxClients && !bFlagDropped; i++)
		{
			...
			if (pPlayer && pPlayer->IsNetClient())
			{
				...
				{
					m_hAssociatedFlag->Drop( pTFPlayer, false );
					bFlagDropped = true;
				}
			}
		}
	}
	m_hAssociatedFlag->Reset();
	m_hAssociatedFlag->SetDisabled( !bEnabled );
}
