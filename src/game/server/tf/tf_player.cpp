...
void CTFPlayer::MedicRegenThink( void )
{
	if( IsPlayerClass( TF_CLASS_MEDIC ) )
	{
		if( IsAlive() )
		{
			...
			int iAmmoAmount = min( TF_MEDIC_REGEN_AMMO, m_Shared.MaxAmmo( TF_AMMO_SECONDARY ) - GetAmmoCount( TF_AMMO_SECONDARY ) );
			GiveAmmo( iAmmoAmount, TF_AMMO_SECONDARY, true );
		}
...
void CTFPlayer::Regenerate( void )
{
	...
	{
		CTFWeaponBaseGrenade* pGrenade = ( CTFWeaponBaseGrenade* )Weapon_OwnsThisID( m_hOffHandWeapon.Get()->GetWeaponID() );
		if (pGrenade)
		{
...
void CTFPlayer::GiveDefaultItems()
{
	...
	int max;
	if ( TFGameRules()->DisableGrenadeRegen() || ( m_pSpawnPoint && !m_pSpawnPoint->SpawnGrenades() ) )
...
void CTFPlayer::RemoveAllAmmo()
{
	...
	// Remove grenades
	for ( i; i < TF_AMMO_COUNT; i++ )
	{
		if( !m_pSpawnPoint || m_pSpawnPoint && m_pSpawnPoint->SpawnGrenades() )
			m_iAmmo.Set( i, 0 );
	}
}
...
void CTFPlayer::HandleCommand_JoinClass( const char *pClassName )
{
	...
	int iChosenClass = iClass == TF_CLASS_RANDOM ? random->RandomInt( TF_FIRST_NORMAL_CLASS, TF_LAST_NORMAL_CLASS ) : iClass;

	// joining the same class?
	if( iClass != TF_CLASS_RANDOM && iChosenClass == GetDesiredPlayerClassIndex() )
	{
...
void CTFPlayer::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
	...
	if (info_modified.GetDamageType() & DMG_USE_HITLOCATIONS)
	{
		...
		case HITGROUP_RIGHTLEG:
		{
			...
			if (pWpn && pWpn->CanFireCriticalShot( true ))
			{
				m_Shared.AddCond( TF_COND_LEG_DAMAGED, 6.0f );
			}
...
int CTFPlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if( m_Shared.InCond( TF_COND_INVULNERABLE ) || ( bitsDamage & DMG_SONIC ) || bNoGrenadeDamage )
	{
		...
		if( !bAllowDamage )
		{
			...
			// Burn sounds are handled in ConditionThink() and concussion blasts don't hurt
	...
	// Deal with armor if we have to
	if ( pf_armor_enabled.GetBool() )
	{
		float flArmorBonus = pf_armor_bonus.GetFloat();

		if ( info.GetDamageType() & DMG_BLAST )
		{
			// blasts damage armor more.
			flArmorBonus *= pf_armor_bonus_multi_explosive.GetFloat();
		}

		if ( info.GetDamageCustom() == TF_DMG_CUSTOM_HEADSHOT )
		{
			flArmorBonus *= pf_armor_bonus_multi_headshots.GetFloat();
		}

		float flArmorRatio = pf_armor_ratio.GetFloat();

		int armortype = GetPlayerClass()->GetArmorType();
		Assert( armortype > ARMOR_NONE );
		switch ( armortype )
		{
			case ARMOR_LIGHT:
				flArmorRatio = pf_armor_ratio_light.GetFloat();
				break;
			case ARMOR_MEDIUM:
				flArmorRatio = pf_armor_ratio_medium.GetFloat();
				break;
			case ARMOR_HEAVY:
				flArmorRatio = pf_armor_ratio_heavy.GetFloat();
				break;
		}

		// Armor. Armor is ignored if infection damage, or is a backstab
		if ( !info.IgnoreArmor() && ArmorValue() > 0 && !( info.GetDamageType() & ( DMG_INFECTION | DMG_FALL ) ) && !( info.GetDamageCustom() == TF_DMG_CUSTOM_BACKSTAB ) )
		{
			// Damage dealt to player
			float flNewDamage = info.GetDamage() * ( 1.0f - flArmorRatio );

			float flCurrentArmor = ArmorValue();
			float flArmorRemoved = ( info.GetDamage() - flNewDamage ) * flArmorBonus;

			// Does this use more armor than we have?
			if ( flArmorRemoved > flCurrentArmor )
			{
				flCurrentArmor *= ( 1 / flArmorBonus );
				flNewDamage = info.GetDamage() - flCurrentArmor;
				flArmorRemoved = ArmorValue();
				SetArmorValue( 0 );
			}
			else
				SetArmorValue( flCurrentArmor - ( int )( flArmorRemoved + 0.5 ) );
...
int CTFPlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( bIgniting )
	{
		m_Shared.Burn( ToTFPlayer( pAttacker ), info.GetDamageCustom() == TF_DMG_CUSTOM_NAPALM_BURNING );
	}
	...
	if( event )
	{
		...
		if (info.GetAttacker() && info.GetAttacker()->IsPlayer())
		{
			event->SetInt("attacker", ToTFPlayer(info.GetAttacker())->GetUserID());
		}
...
void CTFPlayer::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	...
	if( pVictim->IsPlayer() )
	{
		...
		else if( info.GetDamageCustom() == TF_DMG_CUSTOM_BURNING  || info.GetDamageCustom() == TF_DMG_CUSTOM_NAPALM_BURNING )
		{
...
void CTFPlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ((( info.GetDamageType() & DMG_DISSOLVE ) != 0 ))
		bEMP = true;
	...
	if( bGib || bRagdoll )
	{
		CreateRagdollEntity( bGib, bBurning, bEMP );
	}
...
bool CTFPlayer::PlayDeathAnimation( const CTakeDamageInfo &info, CTakeDamageInfo &info_modified )
{
	...
	{
		if( GetPlayerClass()->IsClass( TF_CLASS_HEAVYWEAPONS ) && GetActiveTFWeapon()->GetSlot() == 0)
		{
			bPlayDeathAnim = true;
		}
	}
	...
	{
		if (GetPlayerClass()->IsClass( TF_CLASS_SNIPER ) && GetActiveTFWeapon()->GetSlot() == 0)
		{
			bPlayDeathAnim = true;
		}
	}
...
void CTFPlayer::TeamFortress_RemoveRockets( void )
{
	...
	// Putting nails here as it's close enough
	RemoveOwnedEnt( "tf_projectile_nail" );
	RemoveOwnedEnt( "tf_projectile_syringe" );
	RemoveOwnedEnt( "tf_projectile_dart" );
}
...
{
	RemoveOwnedEnt( "tf_weapon_grenade_caltrop_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_concussion_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_emp_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_gas_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_heal_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_mirv_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_mirv_bomb", true );
	RemoveOwnedEnt( "tf_weapon_grenade_nail_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_napalm_projectile", true );
	RemoveOwnedEnt( "tf_weapon_grenade_normal_projectile", true );
}
...
void CTFPlayer::ForceRespawn( void )
{
	...
	int iChosenClass = iDesiredClass == TF_CLASS_RANDOM ? random->RandomInt( TF_FIRST_NORMAL_CLASS, TF_LAST_NORMAL_CLASS ) : iDesiredClass;

	if( iChosenClass == TF_CLASS_UNDEFINED )
	{
		return;
	}
	...
	if( GetPlayerClass()->GetClassIndex() != iChosenClass )
	{
		// clean up any pipebombs/buildings in the world (no explosions)
		TeamFortress_RemoveEverythingFromWorld();

		GetPlayerClass()->Init( iChosenClass );
...
//-----------------------------------------------------------------------------
void CTFPlayer::CreateRagdollEntity( bool bGib, bool bBurning, bool bDisintegrate )
{
