...
ConVar bot_runabout( "bot_runabout", "0", FCVAR_CHEAT );
...
void Bot_Think( CTFPlayer *pBot )
{
	...
	else if ( pBot->IsAlive() && (pBot->GetSolid() == SOLID_BBOX) )
	{
		...
		if (!pBot->IsEFlagSet( EFL_BOT_FROZEN ) && bot_runabout.GetBool())
		{
			Vector vecEnd;
			Vector forward;
...
//------------------------------------------------------------------------------
// Purpose: change the specified cvar of a bot
//------------------------------------------------------------------------------
void cc_bot_changecvar(const CCommand& args)
{
	if (args.ArgC() < 3)
	{
		Msg("Too few parameters.  Usage: bot_command <bot name> <cvar> <value>\n");
		return;
	}
	...
	int iSize = Q_strlen(cvar) + 1;
	char* pBuf = (char*)malloc(iSize);
	Q_snprintf(pBuf, iSize, "%s", cvar);
	...
	int iSizea = Q_strlen(value) + 1;
	char* pBufa = (char*)malloc(iSizea);
	Q_snprintf(pBufa, iSizea, "%s", value);
	...
	// make a command object with the intended command line
	engine->SetFakeClientConVarValue(pEdict, cvar, value);
}
static ConCommand bot_changecvar("bot_cvar", cc_bot_changecvar, "<bot id> <command string...>.  Changes the specified cvar on behalf of specified bot", FCVAR_CHEAT);
...
CON_COMMAND_F( bot_teleport_to_player, "Teleport the specified bot to you.\n\tFormat: bot_teleport_me <bot name> <player name>", FCVAR_CHEAT )
{
	if (args.ArgC() < 2)
	{
		Msg( "Too few parameters.  bot_teleport <bot name> <player name>\n" );
		return;
	}
	...
	for (int i = 1; i <= gpGlobals->maxClients+1; i++)
	{
		...
		{
			Msg( "No player with name %s\n", args[1] );
			return;
		}
		CBasePlayer *pTPlayer = ToBasePlayer( UTIL_PlayerByIndex( i ) );
		if (pTPlayer)
		{
			if (Q_strstr( pTPlayer->GetPlayerName(), args[2] ))
			{
				vecAng = pTPlayer->EyeAngles();
				vecPos = pTPlayer->GetAbsOrigin();
				break;
			}
		}
	}
