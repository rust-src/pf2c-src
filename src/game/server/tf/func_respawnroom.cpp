...
// Purpose: 
//-----------------------------------------------------------------------------
bool CFuncRespawnRoom::AllowGrenades() const
{
	return m_bAllowGrenades;
}
...
// Purpose: Is a given point contained within a respawn room?
//-----------------------------------------------------------------------------
bool GrenadeInRespawnRoom( CBaseEntity *pEntity, const Vector &vecOrigin )
{
	CBaseEntity *pTempEnt = NULL;
	while( (pTempEnt = gEntList.FindEntityByClassname( pTempEnt, "func_respawnroom" )) != NULL )
	{
		CFuncRespawnRoom *pRespawnRoom = dynamic_cast<CFuncRespawnRoom *>(pTempEnt);

		if( pRespawnRoom->GetActive() && pRespawnRoom->PointIsWithin( vecOrigin ) )
		{
			if (pRespawnRoom->AllowGrenades() || 
