...
void CObjectDispenser::DispenseThink( void )
{
	...
	if ( m_flNextAmmoDispense <= gpGlobals->curtime )
	{
		...
		for ( int i=0;i<iNumberOfNearbyEntities;i++ )
		{
			...
			if (pPlayer->GetTeamNumber() != GetBuilder()->GetTeamNumber())
			{
	...
	for ( int i = iSize-1; i >= 0; i-- )
	{
		...
		if ( pf_armor_enabled.GetBool() && bHealingTarget && bValidHealTarget )
		{
			CTFPlayer* pPlayer = ToTFPlayer( pEnt );
			if ( pPlayer->ArmorValue() < pPlayer->GetPlayerClass()->GetMaxArmor() )
			{
				if ( m_bRepairArmorTick )
				{
					pPlayer->SetArmorValue( pPlayer->ArmorValue() + 1 );
				}
				m_bRepairArmorTick = !m_bRepairArmorTick;
			}
		}
	}
