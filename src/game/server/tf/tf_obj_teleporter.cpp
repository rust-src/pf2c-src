...
bool CObjectTeleporter::InputWrenchHit( CTFPlayer *pPlayer )
{
	bool bRet = BaseClass::InputWrenchHit( pPlayer );
	if ( !pPlayer || !GetMatchingTeleporter() )
		return bRet;

	bool bDidWork = false;

	if( GetMatchingTeleporter()->HasSapper() )
	{
		...
		{
			CBaseObject *pObject = GetMatchingTeleporter()->GetBuildPointObject( iPoint );
	...
	else if ( GetMatchingTeleporter()->IsUpgrading() || GetMatchingTeleporter()->IsBuilding() )
	{
		bDidWork = false;
	}
	else
	{
		...
		bDidWork = GetMatchingTeleporter()->OnWrenchHit( pPlayer );
	}

	return bRet || bDidWork;
}
...
void CObjectTeleporter::TeleporterTouch( CBaseEntity *pOther )
{
	...
	//Lazy!
	if (pBuilder->GetTeamNumber() != pPlayer->GetTeamNumber() && pPlayer->GetPlayerClass()->GetClassIndex() != TF_CLASS_SPY)
	{
		SetEnemyUsing(true);
...
void CObjectTeleporter::TeleporterThink( void )
{
	...
	if (IsEnemyUsing() && m_flNextCheckEnemyUsing <= gpGlobals->curtime)
	{
		SetEnemyUsing(false);
	}
...
{
	float flDamage = min(50 + m_iHealth, 150);
	ExplosionCreate(
		GetAbsOrigin(),
		GetAbsAngles(),
		GetBuilder(),
