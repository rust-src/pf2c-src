...
IMPLEMENT_SERVERCLASS_ST(CBaseObject, DT_BaseObject)
	...
	SendPropBool(SENDINFO(m_bEnemyUsing)),
...
void CBaseObject::Precache()
{
	...
	PrecacheScriptSound("Breakable.Spark");
...
void CBaseObject::Spawn( void )
{
	...
	m_bHasSapper = false;
	m_flDisableTime = gpGlobals->curtime + 1e16; //prevents a sound issue
...
int CBaseObject::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	for ( int iPoint=0;iPoint<iNumObjects;iPoint++ )
	{
		...
		if ( !pObject || pObject->IsHostileUpgrade() )
		{
			if ( pObject == info.GetInflictor() )
				bSapperInflictor = true;
			continue;
		}
	...
	IGameEvent *event = gameeventmanager->CreateEvent("object_hurt");
	if (event)
	{
		...
		event->SetInt( "damageamount", (int)(flDamage + 0.5f) );
	...
	if ( GetHealth() <= 0 )
	{
		if ( info.GetAttacker() )
		{
			// Check if we just shot off a sapper
			if ( info.GetDamageCustom() != TF_DMG_WRENCH_FIX && IsHostileUpgrade() )
			{
				CSingleUserRecipientFilter filter( ToTFPlayer(info.GetAttacker()) );
				UserMessageBegin( filter, "AchievementEvent" );
				WRITE_SHORT( ACHIEVEMENT_PF_DESTROY_SAPPER_GUN );
				MessageEnd();
			}
