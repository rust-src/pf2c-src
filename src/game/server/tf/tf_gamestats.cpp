...
void CTFGameStats::Event_PlayerLeachedHealth( CTFPlayer *pPlayer, bool bDispenserHeal, float amount ) 
{
	if ( !bDispenserHeal )
	{
		...
		if ( 0 == stats.statsAccumulated.m_iStat[TFSTAT_HEALTHLEACHED] )
		{
			...
			WRITE_SHORT( ACHIEVEMENT_TF_GET_HEALED_BYENEMY );
			MessageEnd();
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameStats::Event_PlayerArmorRepairedOther( CTFPlayer* pPlayer, float amount )
{
	IncrementStat( pPlayer, TFSTAT_ARMORRESTORED, ( int )amount );
}
