...
#define SENTRYGUN_FRAG_RESIST_LVL_1		0.15
#define SENTRYGUN_FRAG_RESIST_LVL_2		SENTRYGUN_FRAG_RESIST_LVL_1
#define SENTRYGUN_FRAG_RESIST_LVL_3		SENTRYGUN_FRAG_RESIST_LVL_1
...
ConVar tf_sentrygun_upgrade_per_hit( "tf_sentrygun_upgrade_per_hit", "50", FCVAR_CHEAT );
...
void CObjectSentrygun::StartUpgrading( void )
{
	...
	case 2:
		...
		m_flFragResist = SENTRYGUN_FRAG_RESIST_LVL_2;
	...
	case 3:
		...
		m_flFragResist = SENTRYGUN_FRAG_RESIST_LVL_3;
...
void CObjectSentrygun::Disable(float flTime)
{
	BaseClass::Disable(flTime);
...
int CObjectSentrygun::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if (pGrenade && !pGrenade->IsPillGrenade() && pGrenade->GetWeaponID() == TF_WEAPON_GRENADE_NORMAL)
	{
		float flDamage = newInfo.GetDamage();

		flDamage *= (1.0 - m_flFragResist);
	...
	else if (info.GetAttacker() && info.GetAttacker()->GetTeamNumber() != GetBuilder()->GetTeamNumber())
	{
		// only play if we've taken significant damage.
		// might change this to play at certain percentages
		if(newInfo.GetDamage() > 35)
			EmitSound("Building_Sentry.Damaged");
	}
