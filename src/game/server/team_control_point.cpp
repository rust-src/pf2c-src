...
BEGIN_DATADESC(CTeamControlPoint)
	...
	DEFINE_KEYFIELD(m_iNeedsFlag,				FIELD_INTEGER,	"needs_flag"),
...
{
	return (m_iNeedsFlag == 2) || ((m_iNeedsFlag == 1) && TFGameRules()->IsADFlag());
}
...
void CTeamControlPoint::InternalSetOwner( int iCapTeam, bool bMakeSound, int iNumCappers, int *pCappingPlayers )
{
	...
	SetBodygroup( m_TeamData[m_iTeam].iModelBodygroup, 0);
