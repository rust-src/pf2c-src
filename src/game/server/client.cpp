...
void kill_helper( const CCommand &args, bool bExplode )
{
	...
	if ( args.ArgC() > 1 && (sv_cheats->GetBool()) )
	{
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pTPlayer )
			{
				if ( Q_strstr( pTPlayer->GetPlayerName(), args[1] ) )
				{
					pTPlayer->CommitSuicide( bExplode );
				}
			}
		}
	}
...
CON_COMMAND( give, "Give item to player.\n\tArguments: <item_name>" )
{
		...
		&& args.ArgC() >= 2 )
	{
		...
		if (pEnt)
		{
			CTFWeaponBase* pWeapon = static_cast<CTFWeaponBase*>(pEnt);
			if (pWeapon)
			{
				pWeapon->DefaultTouch(pTFPlayer);
			}
		}
	}
}
...
void CC_HurtMe_f(const CCommand &args)
{
	...
	if ( (args.ArgC() == 1 || args.ArgC() == 0) && (sv_cheats->GetBool()))
	{
		int iDamage = 10;
	...
	else if (args.ArgC() > 1 && (sv_cheats->GetBool()))
	{
		int iDamage = 10;
			...
			for (int i = 1; i <= gpGlobals->maxClients; i++)
			{
				...
				if (pTPlayer)
				{
					if (Q_strstr(pTPlayer->GetPlayerName(), args[2]))
					{
						...
						pTPlayer->TakeDamage(CTakeDamageInfo(pTPlayer, pTPlayer, iDamage, DMG_PREVENT_PHYSICS_FORCE));
					}
				}
			}
		}
