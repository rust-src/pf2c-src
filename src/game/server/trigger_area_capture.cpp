...
void CTriggerAreaCapture::EndCapture( int team )
{
	...
	if ( TFGameRules()->IsADFlag() )
	{
		CTFPlayer *pTFCappingPlayer = ToTFPlayer( pCappingPlayer );
		if ( pTFCappingPlayer )
		{
			if ( pTFCappingPlayer->HasItem() )
			{
				...
				if ( pItem->GetItemID() == TF_ITEM_CAPTURE_FLAG )
				{
					...
					if ( pFlag )
					{
						pFlag->Capture( pTFCappingPlayer, m_hPoint.Get()->GetPointIndex(), pf_cp_flag_reset_on_cap.GetBool() );
					}
				}
			}
		}
