...
void CTFPipebombLauncher::SecondaryAttack( void )
{
	...
	if ( m_iPipebombCount )
	{
		...
		else
		{
			...
			if (m_flNextDetSoundTime <= gpGlobals->curtime)
			{
				// Gross method to get the detsound duration
				const char *wav = soundemitterbase->GetWavFileForSound( GetShootSound( SPECIAL3 ), GENDER_NONE );
				float flDetSoundDuration = enginesound->GetSoundDuration( wav );

				m_flNextDetSoundTime = gpGlobals->curtime + flDetSoundDuration;
