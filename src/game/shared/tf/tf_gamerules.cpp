...
#ifdef GAME_DLL
void pf_cp_flag_helper(IConVar* var, const char* pOldValue, float flOldValue)
{
	CTeamControlPointMaster* pMaster = g_hControlPointMasters.Count() ? g_hControlPointMasters[0] : NULL;
	if (pMaster == NULL) 
		return;
	pMaster->UpdateADFlagMode(flOldValue == 1.0f);
	pMaster->UpdateControlPointsNeedingFlags();
...
#ifdef GAME_DLL
void pf_armor_enabled_helper( IConVar* var, const char* pOldValue, float flOldValue )
{
	if ( TFGameRules() )
...
BEGIN_DATADESC( CTFGameRulesProxy )
	// Inputs.
	DEFINE_KEYFIELD( m_bGrenadeRegen, FIELD_BOOLEAN, "disablegrenaderegen"),
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameRulesProxy::SetGrenadeRegeneration()
{
		TFGameRules()->SetGrenadeRegen( m_bGrenadeRegen );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTFGameRules::DisableGrenadeRegen()
{
	return m_bDisableGrenadeRegeneration;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
void CTFGameRules::Activate()
{
	...
	if (pHybrid)
	{
		...
		SetADFlagMode( pf_cp_flag.GetBool() );
	}
	
}

void CTFGameRules::SetADFlagMode( bool bEnabled )
{
	if (CanSetADFlagMode())
	{
		if (bEnabled)
			m_nADFlagMode.Set( AD_FLAGMODE_ENABLED );
		else
			m_nADFlagMode.Set( AD_FLAGMODE_AVAILABLE );
	}
	else
	{
		m_nADFlagMode.Set( AD_FLAGMODE_NONE );
	}
}

void CTFGameRules::UpdateArmorMode( void )
{
	m_bArmorMode.Set( pf_armor_enabled.GetBool() );
}
...
bool CTFGameRules::ShouldCreateEntity( const char *pszClassName )
{
	...
	// if armor is disabled don't create armor pickups
	if (!pf_armor_enabled.GetBool() && !Q_stricmp( pszClassName, "item_armor" ))
		return false;
...
//	float flHalfRadiusSqr = Square( flRadius / 2.0f );
	...
	for ( CEntitySphereQuery sphere( vecSrc, flRadius ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		...
		UTIL_TraceLine( vecSrc, vecSpot, MASK_RADIUS_DAMAGE, info.GetInflictor(), bHandGrenade ? TF_COLLISIONGROUP_GRENADES : COLLISION_GROUP_PROJECTILE /*Seems like a hack, probably a hack*/, &tr );
...
const char *CTFGameRules::GetKillingWeaponName( const CTakeDamageInfo &info, CTFPlayer *pVictim )
{
	...
	if ( 0 == Q_strcmp( killer_weapon_name, "tf_projectile_flame_rocket" ) )
	{
...
// Check if any "team_control_point_round" have an associated flag
bool CTFGameRules::CanSetADFlagMode(void)
{
	if (GetGameType() == TF_GAMETYPE_CP)
	{
		CTeamControlPointRound* pEnt = nullptr;
		while ((pEnt = dynamic_cast<CTeamControlPointRound*>(gEntList.FindEntityByClassname( pEnt, "team_control_point_round" ))) != NULL)
		{
			if (pEnt)
				return pEnt->FindADFlag();
		}

		// we might have a single stage map with a flag
		CTeamControlPointMaster* pEntMaster = nullptr;
		while (( pEntMaster = dynamic_cast<CTeamControlPointMaster*>( gEntList.FindEntityByClassname( pEnt, "team_control_point_master" ) ) ) != NULL)
		{
			if (pEntMaster)
				return pEntMaster->FindADFlag();
		}
	}
	return false;
}
bool CTFGameRules::IsADFlag()
{
	return pf_cp_flag.GetBool() && CanSetADFlagMode();
}
#endif  // GAME_DLL
...
bool CTFGameRules::PlayerMayCapturePoint( CBasePlayer *pPlayer, int iPointIndex, char *pszReason /* = NULL */, int iMaxReasonLength /* = 0 */ )
{
	...
	if (m_iControlPointsNeedingFlags.Get(iPointIndex))
	{
...
bool CTFGameRules::IsAprilFools()
{
	if (IsX360())
		return false;
	if (pf_aprilfools.GetBool())
	{
		m_iAprfoolsMode = APRFOOLS_ON;
		return true;
	}
	if (m_iAprfoolsMode == APRFOOLS_RECALCULATE)
	{
		m_iAprfoolsMode = APRFOOLS_OFF;
		time_t ltime = time(0);
		const time_t* ptime = &ltime;
		struct tm* today = localtime(ptime);
		if (today)
	...
	return (m_iAprfoolsMode == APRFOOLS_ON);
}

bool CTFGameRules::IsMay()
{
	time_t ltime = time(0);
	const time_t *ptime = &ltime;
	struct tm* today = localtime(ptime);
...
{
	m_iControlPointsNeedingFlags.Set(index, flagneeding);
}
