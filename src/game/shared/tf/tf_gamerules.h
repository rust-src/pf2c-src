...
#ifdef GAME_DLL
	DECLARE_DATADESC();
	void	SetGrenadeRegeneration( void );
...
public:
	...
	virtual const unsigned char *GetEncryptionKey( void ) { return (unsigned char *)"/)ZJDSu*"; }
...
public:
	...
	bool			DisableGrenadeRegen( void );

	bool CanSetADFlagMode( void );
	bool IsADFlag(void);
	void SetADFlagMode( bool bEnabled );

	void UpdateArmorMode( void );
...
public:
	...
	virtual bool IsCTFCPHybrid( void ){ return m_bCTFCPHybrid; }
	//virtual bool IsADFlagActive( void ){ return m_bADFlagActive; }
	virtual int GetADFlagMode( void ){ return m_nADFlagMode; }
	virtual bool GetArmorMode( void ) { return m_bArmorMode; }
	...
	void SetControlPointNeedingFlag(int index, bool flagneeding);
	...
	void	ExtendLevel(float minutes);
	void	SetGrenadeRegen( bool bValue ) { m_bDisableGrenadeRegeneration = bValue; }
...
private:
	...
	CNetworkVar( bool, m_bCTFCPHybrid ); // does tf_logic_hybrid_ctf_cp entity exist
	CNetworkVar( int, m_nADFlagMode ); // are we just hybrid_ctf_cp, ad_ compatible or ad_ enabled 
	CNetworkString( m_pszTeamGoalStringRed, MAX_TEAMGOAL_STRING );
	CNetworkString( m_pszTeamGoalStringBlue, MAX_TEAMGOAL_STRING );
	CNetworkArray(bool, m_iControlPointsNeedingFlags, MAX_CONTROL_POINTS);
	CNetworkVar(bool, m_bDisableGrenadeRegeneration );
	CNetworkVar(bool, m_bArmorMode );
