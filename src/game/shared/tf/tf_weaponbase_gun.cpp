...
CBaseEntity *CTFWeaponBaseGun::FireFlameRocket(CTFPlayer *pPlayer)
{
	...
	CTFFlameRocket *pProjectile = CTFFlameRocket::Create(vecSrc, angForward, pPlayer, pPlayer);
	if (pProjectile)
	{
		pProjectile->SetCritical(IsCurrentAttackACrit());
		pProjectile->SetDamage(GetProjectileDamage());
...
void CTFWeaponBaseGun::FireNail( CTFPlayer *pPlayer, int iSpecificNail )
{
	...
	for (int iBullet = 0; iBullet < nBulletsPerShot; ++iBullet)
	{
		...
		{
			case TF_PROJECTILE_SYRINGE:
			vecMuzzle = Vector(20, 6, -6);
			break;
			case TF_PROJECTILE_NAIL:
			vecMuzzle = Vector(25, 5, -8);
			break;
			default:
			case TF_PROJECTILE_DART:
			vecMuzzle = Vector(16, 6, -8);
			break;
		}
		
		GetProjectileFireSetup(pPlayer, vecMuzzle, &vecSrc, &angForward);
		// Add some spread
...
CBaseEntity *CTFWeaponBaseGun::FirePipeBomb( CTFPlayer *pPlayer, bool bRemoteDetonate )
{
	...
	CTFGrenadePipebombProjectile *pProjectile = CTFGrenadePipebombProjectile::Create( vecSrc, pPlayer->EyeAngles() + pPlayer->ConcAngles(), vecVelocity,
...
bool CTFWeaponBaseGun::IsZoomed(void)
{
