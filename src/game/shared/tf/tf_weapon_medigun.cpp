...
//
// Purpose:			The Medic's Medigun weapon
//					
//
// $Workfile:     $
// $Date:         $
...
bool CWeaponMedigun::AllowedToHealTarget( CBaseEntity *pTarget )
{
	...
	if (pTFPlayer->GetUserID() == pOwner->GetUserID())
		return false;

	bool bStealthed = pTFPlayer->m_Shared.InCond( TF_COND_STEALTHED ) || pTFPlayer->m_Shared.InCond( TF_COND_SMOKE_BOMB );
		...
		if ( !m_bChargeRelease )
		{
			if ( pTFPlayer )
			{
				...
				if ( weapon_medigun_charge_rate.GetFloat() )
				{
					...
					else if ( TFGameRules() && TFGameRules()->InSetup() )
					{
						flChargeAmount *= 2.0f;
					}
...
#ifdef GAME_DLL
		if ( m_bHealingSelf && m_hHealingTarget )
		{
			m_bHealingSelf = false;
			pOwner->m_Shared.StopHealing( pOwner );
		}
...
void CWeaponMedigun::ItemPostFrame( void )
{
	...
	if ( m_bChargeRelease && !m_bHealingSelf && !m_hHealingTarget )
	{
...
void CWeaponMedigun::OnDataChanged( DataUpdateType_t updateType )
{
	...
	ManageChargeEffect();
	/*result = (C_TFPlayer *)(*(int (__cdecl **)(C_BaseCombatWeapon *))(*(_DWORD *)a3 + 1696))(a3); // dunno
  if ( (_BYTE)result )
  {
    v25 = C_TFPlayer::GetLocalTFPlayer();
    if ( v25 && C_TFPlayer::IsPlayerClass(result, 5))
    {
        if ( v25 == C_BaseCombatWeapon::GetOwner(a3) )
        {
          if (autocaller_convar.GetBool) )
           C_WeaponMedigun::UpdateMedicAutoCallers(a1, a2, a3); // probably void looking at the actual function
        }
    }
  }*/
}
