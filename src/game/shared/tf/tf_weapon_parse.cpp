...
void CTFWeaponInfo::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_nDamageArmor			= pKeyValuesData->GetInt( "Armor_Damage", m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_nDamage );
	...
	m_WeaponData[TF_WEAPON_SECONDARY_MODE].m_nDamageArmor		= pKeyValuesData->GetInt( "Secondary_Armor_Damage", m_WeaponData[TF_WEAPON_SECONDARY_MODE].m_nDamage);
	...
	{
		char *sClassName = GetPlayerClassData(i)->m_szClassName;
		char buf[128];
		V_snprintf( buf, sizeof( buf ), "ViewModel_%s", sClassName );
		const char *sViewModelName = pKeyValuesData->GetString( buf, nullptr );
		if (sViewModelName != nullptr)
		{
			if (!m_pClassViewModelKV)
			{
				m_pClassViewModelKV = new KeyValues( "class_viewmodels" );;
			}
			m_pClassViewModelKV->SetString( sClassName, sViewModelName );
		}
	}*/
...
const float CTFWeaponInfo::GetWeaponDamage(int iWeapon)
{
	if (pf_armor_enabled.GetBool())
		return m_WeaponData[iWeapon].m_nDamageArmor;
