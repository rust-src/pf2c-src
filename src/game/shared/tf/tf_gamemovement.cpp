...
void CTFGameMovement::FinishDuck()
{
	BaseClass::FinishDuck();

	CTFPlayer *pPlayer = ToTFPlayer(player);
	if (pPlayer && !pPlayer->GetGroundEntity())
		++pPlayer->m_Shared.m_nAirDuckCount;
}
...
void CTFGameMovement::ProcessMovement( CBasePlayer *pBasePlayer, CMoveData *pMove )
{
	...
	mv->m_flMaxSpeed = pf_armor_enabled.GetBool() ? TF_MAX_SPEED_ARMOR : TF_MAX_SPEED; /*tf_maxspeed.GetFloat();*/
...
void CTFGameMovement::AirDash( void )
{
	...
	AngleVectors( QAngle(mv->m_vecViewAngles.x, mv->m_vecViewAngles.y, 0), &vecForward, &vecRight, NULL );
	vecForward.z = 0.0f;
	vecRight.z = 0.0f;		
...
bool CTFGameMovement::CheckJumpButton()
{
	...
	// Cannot jump while revving minigun
	if (m_pTFPlayer->m_Shared.InCond(TF_COND_AIMING))
	{
		CTFWeaponBase* pTFWeapon = m_pTFPlayer->GetActiveTFWeapon();
		if(pTFWeapon->GetWeaponID() == TF_WEAPON_MINIGUN)
			return false;
	}

	// Cannot jump building a detpack
	if ( m_pTFPlayer->m_Shared.InCond( TF_COND_BUILDING_DETPACK ) )
	{
		return false;
	}
	...
	if (!pf_disable_bunnyhop_cap.GetBool())
		PreventBunnyJumping();
...
void CTFGameMovement::WaterMove( void )
{
	...
	AngleVectors( QAngle(mv->m_vecViewAngles.x, mv->m_vecViewAngles.y, 0), &vecForward, &vecRight, &vecUp );
...
void CTFGameMovement::AirMove( void )
{
	...
	AngleVectors (QAngle(mv->m_vecViewAngles.x, mv->m_vecViewAngles.y, 0), &forward, &right, &up);  // Determine movement angles
...
void CTFGameMovement::CheckWaterJump( void )
{
	...
	AngleVectors( QAngle(mv->m_vecViewAngles.x, mv->m_vecViewAngles.y, 0), &forward, &right, NULL );  // Determine movement angles
...
void CTFGameMovement::CheckFalling( void )
{
	...
	if ( player->GetGroundEntity() != NULL && !IsDead() )
	{
		...
		m_pTFPlayer->m_Shared.m_nAirDuckCount = 0;
	}
...
void CTFGameMovement::Duck( void )
{
	...
	CTFPlayer *pPlayer = ToTFPlayer(player);
	if (pPlayer && !pPlayer->GetGroundEntity() && pPlayer->m_Shared.m_nAirDuckCount == TF_AIR_DUCK_LIMIT)
		mv->m_nButtons &= ~IN_DUCK;
