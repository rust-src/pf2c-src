...
void TFPlayerClassData_t::ParseData( KeyValues *pKeyValuesData )
{
	...
	m_nMaxHealthArmor = pKeyValuesData->GetInt( "health_armor_max" );
	m_nMaxArmor = pKeyValuesData->GetInt( "armor_max" );
	m_nArmorType = clamp( pKeyValuesData->GetInt( "armortype" ), ARMOR_LIGHT, ARMOR_HEAVY );
...
int CTFPlayerClassShared::GetMaxHealth(void)
{
	if ( TFGameRules() && TFGameRules()->GetArmorMode() )
		return GetPlayerClassData( m_iClass )->m_nMaxHealthArmor;
	else
