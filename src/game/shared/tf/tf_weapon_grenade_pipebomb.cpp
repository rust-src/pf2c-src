...
{
	if (pf_projectilelight.GetBool() && m_iType != TF_GL_MODE_REMOTE_DETONATE)
	{
...
void CTFGrenadePipebombProjectile::PipebombTouch( CBaseEntity *pOther )
{
	...
	if (m_iType == TF_GL_MODE_REGULAR && pOther->GetTeamNumber() && pOther->GetTeamNumber() != GetTeamNumber() && pOther->m_takedamage != DAMAGE_NO)
	{
		...
		m_flDamage = m_bTouched ? 2*(m_flFullDamage/3.0f) : m_flFullDamage;
...
int CTFGrenadePipebombProjectile::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if (pInflictor)
	{
		CTFWeaponBaseGrenadeProj* pGrenade = dynamic_cast<CTFWeaponBaseGrenadeProj*>(pInflictor); //other grenades dont push
		if (!bSameTeam && !pGrenade || !bSameTeam && pGrenade && pGrenade->IsPillGrenade())
		{
			/*CTakeDamageInfo newInfo = info;
			newInfo.SetDamageForce(info.GetDamageForce() / 5);
			...
			if (info.GetDamageType() != DMG_BLAST && info.GetDamageType() != DMG_BLAST_SURFACE && pWeapon && pWeapon->GetWeaponID() != TF_WEAPON_FLAMETHROWER_ROCKET && pWeapon->GetWeaponID() != TF_WEAPON_ROCKETLAUNCHER)
			{
				SetThink(&CTFGrenadePipebombProjectile::Detonate);
				SetNextThink(gpGlobals->curtime);
			}
