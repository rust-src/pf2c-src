...
void CTFWeaponBaseMelee::Swing( CTFPlayer *pPlayer )
{
	...
	//Don't play a crit sound if we have a knife
	if( IsCurrentAttackACrit() && !bKnife )
	{
		WeaponSound( BURST );
	}
	else
...
bool CTFWeaponBaseMelee::DoSwingTrace( trace_t &trace )
{
	...
	bool bIgnoreTeamTrace = false;

	trace_t first, second;

	UTIL_TraceLine( vecSwingStart, vecSwingEnd, MASK_SOLID, &filter, &first );
	UTIL_TraceLine( vecSwingStart, vecSwingEnd, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &second );

	if (first.fraction < second.fraction)
		bIgnoreTeamTrace = true;
	trace = first;
