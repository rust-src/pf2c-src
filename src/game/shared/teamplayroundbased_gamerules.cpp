...
CTeamplayRoundBasedRules::CTeamplayRoundBasedRules( void )
{
	...
	m_iLastArmorConvarValue = pf_armor_enabled.GetInt();
...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING( void )
{
	...
	if (m_iLastArmorConvarValue != pf_armor_enabled.GetInt())
	{
		m_iLastArmorConvarValue = pf_armor_enabled.GetInt();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::CreateSoldierStatue( void )
{
	if (TFGameRules()->IsMay())
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::MapEntsSpawnAll( void )
{
		...
		pKeyValue->LoadFromFile( filesystem, "scripts/map_ents.txt", "MOD" ) ))
	{
		...
		if (pMap)
		{
			FOR_EACH_SUBKEY( pMap, pSubKey )
			{
				...
				// Don't create armor pickups if armor isn't enabled
				if (!pf_armor_enabled.GetBool() && FStrEq( pSubKey->GetName(), "item_armor" ))
				{
					continue;
				}
				...
				if (pNewMapEnt)
				{
					FOR_EACH_VALUE( pSubKey, pValue )
					{
						...
						{
							Q_snprintf( cModelName, sizeof( cModelName ), pValue->GetString() );
						}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::MapEntsReload( void )
{
